/**
 * Auto-Save System for Presentation Editor - v7.5.30
 *
 ...
 * - Added data_arch_data collection in collectDiagrams() for DATA_ARCHITECTURE persistence
 * - DATA_ARCHITECTURE state (entities, relationships) is now saved via postMessage from iframe
 *
...
(function() {
  ...
  function collectCharts(slideElement, slideIndex, slideId) {
    ...
    chartElements.forEach(el => {
      ...
      // v7.5.37: Skip orphaned elements, but trust charts physically on this slide
      // Charts restored from backend may have stale parent_slide_id if slide IDs regenerated
      if (elementParentId && slideId && elementParentId !== slideId) {
        // Check if chart is actually on this slide (DOM containment check)
        if (!slideElement.contains(el)) {
          console.warn(`[AutoSave] Skipping orphaned chart ${el.id}: parent=${elementParentId}, slide=${slideId}`);
          return;
        }
        // Chart is physically on slide but has stale parent_slide_id - update it
        console.log(`[AutoSave] Updating stale parent_slide_id for ${el.id}: ${elementParentId} → ${slideId}`);
  ...
  function collectDiagrams(slideElement, slideIndex, slideId) {
    ...
    diagramElements.forEach(el => {
      ...
      // v7.5.30: Get data_arch_data if stored as data attribute (for DATA_ARCHITECTURE persistence)
      if (el.dataset.dataArchData) {
        try {
          diagram.data_arch_data = JSON.parse(el.dataset.dataArchData);
          console.log('[AutoSave] Collected DATA_ARCHITECTURE state for:', diagram.id);
        } catch (e) {
          console.warn(`[AutoSave] Failed to parse data_arch_data for ${el.id}:`, e);
        }
      }
