...
function renderL27(content, slide = {}) {
  ...
  return `
    <section data-layout="L27" class="content-slide grid-container" style="${backgroundStyle}">
      ...
      <div class="slide-title" style="grid-row: 2/3; grid-column: 13/32; font-size: 42px; font-weight: bold; color: #1f2937; line-height: 1.2;">
        ${content.slide_title || ''}
      </div>

      <!-- Right: Subtitle (24px, matching L25) -->
      <div class="subtitle" style="grid-row: 3/4; grid-column: 13/32; font-size: 24px; color: #6b7280; line-height: 1.4; margin-top: 8px;">
        ${content.element_1 || ''}
      </div>

      <!-- Right: Main Content Body -->
      <div class="text-content" style="grid-row: 5/17; grid-column: 13/32; font-size: 20px; color: #374151; line-height: 1.6; overflow-y: auto;">
        ${content.main_content || ''}
      </div>

      <!-- Footer: Presentation Name (18px, matching L25) -->
      ${content.presentation_name ? `
      <div class="footer-presentation-name" style="grid-row: 18/19; grid-column: 13/18; padding: 8px 14px; font-size: 18px; color: #1f2937; font-weight: 500; display: flex; align-items: center; height: 100%;">
        ${content.presentation_name}
      </div>
      ` : ''}

      <!-- Footer: Company Logo (bottom-right) -->
